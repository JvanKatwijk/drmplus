...
struct timeval  tv;
	...
	samplesinFile		= ftell (filePointer) / 4;
	sampleRate		= 192000;
	readerOK		= true;
	currPos			= 0;
	running. store (false);
	...
	try {
	   while (running. load()) {
	      ...
	      if (++teller >= 20) {
	         ...
	         float currentTime	= (float)xx / (4 * 192000);
	         float totalTime	= (float)samplesinFile / 192000;
	         set_progressBar (progress * 100, currentTime, totalTime);
                 teller = 0;
	      }
...
//
//	samples - IQ - are 2 * 2 bytes 
void	rawReader_16::setFileat     (int32_t f) {
int newPos	= (f * samplesinFile / 100) / 4;
	fseek (filePointer, newPos * 4, SEEK_SET);
}

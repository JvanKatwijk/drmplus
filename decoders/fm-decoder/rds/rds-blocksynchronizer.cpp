...
#include	"fm-decoder.h"
	...
	rdsBlockSynchronizer::rdsBlockSynchronizer (fmDecoder *RI) {
	myDecoder		= RI;
	crcFecEnabled		= true; 
	
	this -> reset ();
	connect (this, SIGNAL (setRDSisSynchronized (bool)),
	         myDecoder, SLOT (setRDSisSynchronized (bool)));
	connect (this, SIGNAL (setbitErrorRate (double)),
	         myDecoder, SLOT (setbitErrorRate (double)));
}
...
float	rdsBlockSynchronizer::getBitErrorRate	(void) {
...
//	calc BER
	rdsbitErrorRate	= (float)rdsNumofBitErrors / rdsBitsProcessed;

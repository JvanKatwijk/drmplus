...
float	sigmaq_noise	= pow (10.0, - sigmaq_noise_list [0] / 10.0);
float	**PHI;
float	*THETA;
...
//	first shorthands 
	...
	windowsinFrame		= symbolspergroup;
//
...
//	en te vermenigvuldigen met  het aantal symbols per window
	...
	theTrainers		. resize (symbolspergroup);
...
//	We build up the "trainers" as relative addresses of the pilots
	for (window = 0; window < symbolspergroup; window ++) {
...
//	add the noise to the diagonals
	   for (trainer_1 = 0; trainer_1 < trainers_in_window; trainer_1++) {
	      ...
	      std::complex<float> v = getGainValue (sym + window, car);
...
//	and finally, the estimators
	...
	for (i = 0; i < symbolsinFrame; i ++)
	   Estimators [i] = new estimator_1 (refFrame, i);
}
...
//	W_symbol_blk is a matrix with three dimensions
	for (int window = 0; window < symbolspergroup; window ++) { 
	   for (i = 0; i < carriersinSymbol; i ++)
...
//	apply formula 5.40 from the Tsai book to get the SCO
	   if (isGainCell (newSymbol, carrier)) {
...
//	symbols with the same pilot layout
	   if (isGainCell (newSymbol, carrier) ||
	                  isBoostCell (newSymbol, carrier)) {
	      int16_t helpme = realSym (newSymbol - 2 * periodforSymbols);
...
//	Formula 5.26 (page 99, Tsai et al), average phase offset
	      ...
	      prev_2 = getGainValue (newSymbol, carrier);
	   }
	}
//
//	the SCO is then
...
//	The measured offset is in radials
	*sampleclockOffset = offsa / (2 * M_PI * (float (Ts_t) / Tu_t) * offsb);
	...
	for (carrier = K_min; carrier <= K_max; carrier ++) {
	   ...
	   if (isGainCell (newSymbol, carrier))
	      pilotEstimates [newSymbol][indexFor (carrier)] =
	                    testRow [indexFor (carrier)] /
	                             getGainValue (newSymbol, carrier);
	}

//	fprintf (stderr, "adding new symbol %d\n", newSymbol);
//	For equalizing symbol X, we need the pilotvalues
//	from the symbols X - symbols_to_delay .. X + symbols_to_delay - 1
//
//	We added the symbol at loc newSymbol, so we can equalize
...
int16_t	ntwee			= windowBase % symbolspergroup;
...
//	      if (!isGainCell (actualSymbol, pilotCarrier))
//	         fprintf (stderr, "?");
...
//	and normalize the value
	...
	if (symbol == 39) {
	   eqBuffer -> putDataIntoBuffer (refFrame [symbol], nrCarriers);
	   show_eqsymbol (nrCarriers);
	}
...
//	entry in the refFrame, so let us equalize
	...
	for (carrier = K_min; carrier <= K_max; carrier ++) {
	   if (isGainCell (symbol, carrier)) {
	      teller ++;
	      theError += abs (getGainValue (symbol, carrier) *
	                          conj (getGainValue (symbol, carrier))) -
	                   abs (getGainValue (symbol, carrier) *
	                         conj (outVector [indexFor (carrier)]. signalValue));
	   }
	}

//	if (symbol == 39)
//	   fprintf (stderr, " the error = %f\n", theError / teller);
}
